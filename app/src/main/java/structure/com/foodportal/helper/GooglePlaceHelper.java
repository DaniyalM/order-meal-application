    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) throws IOException {
        if (requestCode == GooglePlaceHelper.REQUEST_CODE_AUTOCOMPLETE && data != null) {
            ...
            if (resultCode == PlaceAutocomplete.RESULT_ERROR) {
                ...
                country = PlaceAutocomplete.getPlace(context, data).getLocale().getCountry();
                city = PlaceAutocomplete.getPlace(context, data).getLocale().getDisplayCountry();
...
//                Log.d(TAG, "onActivityResult MAP: Display Name = " + place.getLocale().getDisplayName());
                ...
                googlePlaceDataInterface.onPlaceActivityResult(longitude, latitude, address, "","");
